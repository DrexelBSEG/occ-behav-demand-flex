...
persistent recv time_recv T_GW DayofYear
...
if isempty(CollName)
    ...
    ZoneInfoLabel={'T_out','Tdp_out','RH_out',...
        'Qsen_z','Qlat_z','T_z','Tdp_z','w_z','RH_z','w_out','T_w'};
...
if isempty(T_GW)
    T_GW_all=[67,47,52,67]; % The ground water temperature for the four location
    settings=readtable('settings.csv');
    Location = settings.Location(1);
    T_GW=(T_GW_all(Location)-32)/1.8;
end
ZoneInfo=[ZoneInfo;T_GW];
