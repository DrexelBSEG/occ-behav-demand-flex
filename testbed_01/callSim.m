...
if timestep==0
    ...
    % add control models to path
    addpath(strcat(par_dir,'\VirtualHP'));
    % database name
    DBName = 'HILFT';
...
%         '_',char(datestr(now,'HHMMSS'))];
    ...
    % Labels
    MeasLabel={'m_sup','T_sup','w_sup','T_z','w_z','T_out_emulated','Power_HVAC','w_sup_received'};
...
if (recv<0.5 || timestep>ts_recv)
    ...
    % push Meas to DB
    % determine the correct w_sup, which is min(w_sup_received, w_z). (YC 2022/10/20)
    Meas(8)=Meas(3);  % Meas(8)=w_sup_received
    Meas(3)=min(Meas(8),Meas(5));  % Meas(3)=the correct w_sup
...
if timestep==0
    ...
    elseif Season_type==1 && (Location==1 || Location==2 || Location==3)  % winter cases (YC 2022/11/10)
        SimulinkName=['OneZone_STD',char(CollName_STD(STD)),'_',...
            char(CollName_Location(Location)),'_',char(CollName_DenOcc(Dense_Occupancy+1)),'_winter.slx'];
    else
        SimulinkName=['OneZone_STD',char(CollName_STD(STD)),'_',...
