...
for n = 1:size(OccupantMatrix,2)
    % At 6 AM on each day's occupancy, reset agent occupancy times (+
    % noise), and morning clothing level
    if (abs(DayTime - ((6-DaylightSaving)/24)) < (1.5/60/24)) % only for the timestep when the first occupant get office (YC 2022/02/11)
    ...
    if OccupantMatrix(n).InOffice == 1
       ...
       % Determine occupant position
        Position = Coordinate(OccupantMatrix(n),ENumInOcc,counter_occ);
...
%         OccupantMatrix(n).IndoorEnvironmentVectorBase(3)=predict(V_Airflow.net,ANN_input);
        ...
        % Update occupant's physiological information  (YC 2022/02/11)
